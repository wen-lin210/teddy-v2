...
async function Running({ message, args, getLang, prefix }) {
    const { type, messageReply, senderID, threadID } = message;
    const replyAttachments = messageReply?.attachments || [];
    const selfAttachments = message?.attachments || [];
    const attachments = type == "message_reply" ? replyAttachments : selfAttachments;
    const rawBody = (type == "message_reply" && messageReply?.body) ? messageReply.body : (message?.body || "");
    let msg = rawBody;
    if (type !== "message_reply" && rawBody) {
        const cutIndex = (prefix?.length || 0) + config.name.length + 1;
        msg = rawBody.length >= cutIndex ? rawBody.slice(cutIndex) : "";
    }
    msg = msg || "";
