...
async function callback( {
  ...
  if (input[0] == "deny" || input[0] == "d") {
    ...
    message.reply(getLang("successDeny", {
      success
    }));
    if (fail.length > 0) message.reply(getLang("failDeny", {
      fail: fail.join("\n")
    }));
  } else {
    if (indexes.length == 0) return message.reply(getLang("invalidIndexes"));
    ...
    for (const thread of threads) {
      ...
      let _info = await message.send(global.config.GBOTWAR_MESSAGE.notiApprove, cTID).then(data => data).catch(handleError);
    ...
    message.reply(getLang("successApprove", {
      success
    }));
    if (fail.length > 0) message.reply(getLang("failApprove", {
      fail: fail.join("\n")
    }));
  }
...
async function Running( {
  ...
  try {
    ...
    const pendingThread = [...SPAM,
      ...PENDING].filter(thread => thread.isGroup && thread.isSubscribed);
    ...
    .then(_ => _.addReplyEvent({
      pendingThread, callback
    }))
