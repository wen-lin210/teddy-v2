...
const fcaLogLevel = (process.env.FCA_LOG_LEVEL || "silent").toLowerCase();

function canLog(level) {
  if (fcaLogLevel === "silent" || fcaLogLevel === "off" || fcaLogLevel === "none") return false;
  if (fcaLogLevel === "error") return level === "error";
  if (fcaLogLevel === "warn") return level === "error" || level === "warn";
  return true;
}
...
/**
 * Parse mentions from a message object.
 * Supports both old format (data.prng) and new Facebook format (messageMetadata.data.data.*.asMap).
 * @param {Object} msgObj - Message object that may contain data.prng or messageMetadata.data.data
 * @param {string} body - Message body text
 * @returns {Object} mentions - Map of userID to mention text
 */
function parseMentions(msgObj, body) {
  const mentions = {};
  if (!msgObj || !body) return mentions;

  // Old format: data.prng
  if (msgObj.data && msgObj.data.prng) {
    try {
      const mdata = JSON.parse(msgObj.data.prng);
      for (let i = 0; i < mdata.length; i++) {
        mentions[mdata[i].i] = body.substring(
          mdata[i].o,
          mdata[i].o + mdata[i].l
        );
      }
  ...
  // New format: messageMetadata.data.data contains mention entries
  if (Object.keys(mentions).length === 0) {
    const md = msgObj.messageMetadata || msgObj;
    if (md && md.data && md.data.data) {
      try {
        const metaData = md.data.data;
        for (const key of Object.keys(metaData)) {
          const val = metaData[key];
          if (val && val.asMap && val.asMap.data) {
            const entries = val.asMap.data;
            for (const idx of Object.keys(entries)) {
              const entry = entries[idx];
              if (entry && entry.asMap && entry.asMap.data) {
                const d = entry.asMap.data;
                if (d.id && d.offset && d.length) {
                  const id = d.id.asLong || d.id.asString;
                  const offset = parseInt(d.offset.asLong || d.offset.asString || "0");
                  const length = parseInt(d.length.asLong || d.length.asString || "0");
                  if (id) {
...
function formatDeltaMessage(m) {
  ...
  const mentions = parseMentions({ data: m.delta.data, messageMetadata: md }, body);
