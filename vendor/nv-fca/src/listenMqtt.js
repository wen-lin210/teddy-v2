...
function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
  ...
  mqttClient.on('error', function (err) {
    utils.error("listenMqtt", "MQTT error: " + (err.message || err));
    if (ctx.globalOptions.autoReconnect) {
      utils.warn("MQTT error detected, attempting reconnect...");
      stopListening();
  ...
  mqttClient.on('close', () => {
    if (ctx.globalOptions.autoReconnect && !ctx._reconnecting) {
      ctx._reconnecting = true;
      utils.warn("MQTT connection closed. Reconnecting in 5s...");
      setTimeout(() => {
        ctx._reconnecting = false;
        getSeqID();
      }, 5000);
    }
  });
  mqttClient.on('disconnect', () => {
    utils.warn("MQTT disconnected by broker.");
  });
}
    ...
    if (clientPayload && clientPayload.deltas) {
      for (var i in clientPayload.deltas) {
        ...
        if (delta.deltaMessageReaction && !!ctx.globalOptions.listenEvents) {
          ...
          //Mention block - #1 (supports both old prng and new messageMetadata format)
          var mentions = utils.parseMentions(
            delta.deltaMessageReply.message,
          ...
          if (delta.deltaMessageReply.repliedToMessage) {
            //Mention block - #2 (supports both old prng and new messageMetadata format)
            var rmentions = utils.parseMentions(
    ...
    defaultFuncs
      ...
      .then((resData) => {
        ...
        if (resData[0].o0.data.viewer.message_threads.sync_sequence_id) {
          ...
          _seqIdRetries = 0; // Reset on success
          listenMqtt(defaultFuncs, api, ctx, globalCallback);
      ...
      .catch((err) => {
        ...
        _seqIdRetries++;
        if (_seqIdRetries <= _maxRetries) {
          var delay = Math.min(1000 * Math.pow(2, _seqIdRetries), 30000);
          utils.warn("getSeqID failed (attempt " + _seqIdRetries + "/" + _maxRetries + "), retrying in " + (delay / 1000) + "s...");
          setTimeout(() => getSeqID(), delay);
        } else {
          utils.error("listenMqtt", "getSeqID failed after " + _maxRetries + " attempts. Giving up.");
          _seqIdRetries = 0;
          return globalCallback(err);
        }
      });
